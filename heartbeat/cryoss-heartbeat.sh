...
get_services_json() {
    ...
    for svc in "${SERVICES[@]}"; do
        local status
        # BUG fix 2026-05-19 : ``$(cmd || echo unknown)`` cumule les deux
        # stdouts quand systemctl retourne un statut transitoire ("activating",
        # "inactive") AVEC un exit code non-zero. On capture seulement la
        # premiere ligne pour eviter d'injecter un newline litteral dans le
        # JSON des services.
        status=$(systemctl is-active "$svc" 2>/dev/null | head -n 1)
...
# Services — meme fix que get_services_json (BUG 2026-05-19) : on prend
# la premiere ligne du stdout et fallback "unknown" si vide, pour eviter
# d'injecter "activating\nunknown" qui casse le JSON downstream.
SVC_SSH=$(systemctl is-active ssh 2>/dev/null | head -n 1); [[ -z "$SVC_SSH" ]] && SVC_SSH="unknown"
SVC_F2B=$(systemctl is-active fail2ban 2>/dev/null | head -n 1); [[ -z "$SVC_F2B" ]] && SVC_F2B="unknown"
SVC_API=$(systemctl is-active cryoss-api 2>/dev/null | head -n 1); [[ -z "$SVC_API" ]] && SVC_API="unknown"
SVC_HEALTH=$(systemctl is-active cryoss-health-daily.timer 2>/dev/null | head -n 1); [[ -z "$SVC_HEALTH" ]] && SVC_HEALTH="unknown"
